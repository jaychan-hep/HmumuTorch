...
def gettingsig(region, variable, boundaries, transform, estimate):
    ...
    for category in ['sig', 'VBF', 'ggF', "data_sid", "bkgmc_sid", "bkgmc_cen"]:
        ...
        for data in tqdm(uproot.iterate(f'outputs/postTrainingNtuples/{region}/{"bkgmc" if "bkgmc" in category else category}.root:test', expressions=[f"{variable}_score{'_t' if transform else ''}", 'm_mumu', 'weight', 'eventNumber'], cut=None if 'sid' in category else "eventNumber%4==3", step_size=500000, library='pd'), desc=f'Loading {category}', bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
...
def main():
    ...
    with open('outputs/significances/%s/%d.json' % (region, args.nbin), 'w') as json_file:
